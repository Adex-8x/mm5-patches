...
void ProcessBottomTrail3d() {
    ...
    for (int i = 0; i < STEP_LEN; i+=1) {
        ...
        if (STEPS[element_place].x_start != 0xFF) {
            ...
            test_command->texture_vram_offset = 0x1F000;
            test_command->palette_base_addr = 0x1F00*2;
            test_command->texture_size.x = 8;
            test_command->texture_size.y = 1;
            test_command->scale.x = 0x1000;
            test_command->scale.y = 0x2000;
            test_command->texture_top_left.x = 8;
            test_command->texture_top_left.y = 6;
            ...
            test_command->texture_params.texture_s_size = 4;
            test_command->texture_params.texture_t_size = 2;
            test_command->texture_params.texture_format = 3;
